...
def test_drift_latest_success_and_missing(client, monkeypatch):
    # Mock get_db_session to return a factory (callable) that creates DummyDBSession
    def mock_session_factory():
        return DummyDBSession(row={"timestamp": "2026-05-04T00:00:00Z", "env": "prod", "report": {"ok": True}})
    
    monkeypatch.setattr(api_main, "get_db_session", lambda: mock_session_factory)
    ...
    def mock_session_factory_empty():
        return DummyDBSession(row=None)
    
    monkeypatch.setattr(api_main, "get_db_session", lambda: mock_session_factory_empty)
...
async def test_drift_latest_db_failure(client, monkeypatch):
    # Mock get_db_session to return a factory (callable) that creates DummyDBSession with failure
    def mock_session_factory():
        return DummyDBSession(should_fail=True)
